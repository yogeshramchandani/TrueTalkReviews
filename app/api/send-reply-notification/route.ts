...
export async function POST(request: Request) {
  try {
    ...
    // 1. Get Review Details (including provider_id)
    const { data: reviewData, error: reviewError } = await supabaseAdmin
      .from('reviews')
      .select('reviewer_id, provider_id, content') // 🎯 Added provider_id
      .eq('id', reviewId)
      .single()
    ...
    // 2. Fetch the Professional's unique username 🎯
    ...
    if (profileError || !profileData?.username) {
       ...
       // Fallback logic: if no username exists, use the ID
    }

    // 3. Get User (Reviewer) Email
    ...
    // 4. Construct Link using the unique Username 🎯
    const baseUrl = 'https://truvouch.app' 
    const identifier = profileData?.username || reviewData.provider_id;
    const profileLink = `${baseUrl}/u/${identifier}` 
      ...
      html: `
        ...
        <body style="margin: 0; padding: 0; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #f4f7f6; color: #333;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 40px 20px; border-radius: 8px; margin-top: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            ...
            <div style="text-align: center; margin-bottom: 30px;">
              <a href="${profileLink}" style="background-color: #0f766e; color: #ffffff; padding: 16px 32px; border-radius: 50px; text-decoration: none; font-weight: bold; font-size: 16px; display: inline-block; box-shadow: 0 4px 10px rgba(15, 118, 110, 0.3);">
                Read the Reply
              </a>
            </div>
